...
public:
    ...
    void paint (juce::Graphics& g) override
    {
        ...
        static const Row rows[] {
            ...
            { "NOISE knob", "white-noise source level (the 4th source, beneath the oscillators)" },
            { "RS / RN / FR (osc)", "note start phase: Reset / Random / Free-run" },
            { "FM 2>1 / FM 3>2 knob", "phase-modulation depth: osc 2 modulates osc 1, osc 3 modulates osc 2" },
            { "FM recipe 1", "set the CARRIER osc's wave to SIN / TRI / WT (SAW/SQR disable FM)" },
            { "FM recipe 2", "raise its FM knob -- a simple carrier grows bell / e-piano sidebands" },
            { "FM recipe 3", "tune the MODULATOR osc's SEMI/OCTAVE for the ratio (0 / +12 harmonic, detune = bell)" },
